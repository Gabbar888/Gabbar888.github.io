...
**The hardest problem — and my favorite.** The win I'm proudest of was untangling **Docker–NVMe storage mismatches**. When you run a custom Docker image for AWS training, the hardware expects certain files and directories to live in specific places — but inside the container, the filesystem can look completely different. Training jobs could behave in a way which makes this problem almost invisible just because the storage the hardware expected wasn't actually mapped to where the container thought it was. Figuring out *which* physical components needed to be attached to *which* paths — and getting the right high-speed NVMe storage wired to the right mount points — was a genuinely tricky debugging problem with no error message coming up and got identified becasue of slowness of transfer speed present in it. (Alongside this, I worked through the usual gauntlet of CUDA out-of-memory errors that come with models this size.)
